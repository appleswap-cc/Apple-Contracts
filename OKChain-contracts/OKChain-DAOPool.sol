...
contract AppleDAOPool is Ownable {
    using SafeMath for uint256;
    ...
    constructor(IERC20 _Appletoken) public { 
    ...
    //添加pool池
    function addPool(IERC20 _token, address _Rewardtoken, uint256 _starttime, uint256 _endtime, uint256 _locktime, uint256 _tokenPertime, bool _withUpdate) public onlyOwner {
        require(_locktime >= 3 days);
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function updatePool(uint256 _pid) public validatePool(_pid) {
        ...
        uint256 Reward = multiplier.mul(pool.tokenPertime);
        pool.acctokenPerShare = pool.acctokenPerShare.add(Reward.mul(1e18).div(totalStake));
        pool.lastRewardtime = block.timestamp < pool.endtime ? block.timestamp : pool.endtime;
    }
    ...
    function pendingReward(uint256 _pid, address _user) public view validatePool(_pid) returns (uint256)  {
        ...
        if (block.timestamp > pool.lastRewardtime && totalStake > 0) {
            uint256 multiplier = getMultiplier(pool);
            uint256 Reward = multiplier.mul(pool.tokenPertime);
            acctokenPerShare = acctokenPerShare.add(Reward.mul(1e18).div(totalStake));
        
        }
        return user.pending.add(user.amount.mul(acctokenPerShare).div(1e18)).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public validatePool(_pid){
        ...
        if (block.timestamp >= pool.starttime){
            user.withdrawtime = block.timestamp.add(pool.locktime);
        
        }else{
            user.withdrawtime = pool.starttime.add(pool.locktime);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public validatePool(_pid){
        PoolInfo storage pool = poolinfo[_pid];
        UserInfo storage user = users[_pid][msg.sender];
        require(block.timestamp >= user.withdrawtime, "the current time less than lock time");
    ...
    function reclaimStakingReward(uint256 _pid) public validatePool(_pid) {
        ...
        UserInfo storage user = users[_pid][msg.sender];
        updatePool(_pid);
        uint256 pending = user.pending.add(user.amount.mul(pool.acctokenPerShare).div(1e18).sub(user.rewardDebt));
        if (pending > 0 && pool.Rewardtoken != address(0)) {
            safetokenTransfer(pool.Rewardtoken, msg.sender, pending);
        }
        if (pending > 0 && pool.Rewardtoken == address(0)){
            safeOKTTransfer(msg.sender, pending);
        }
        user.pending = 0;
        user.depositerewarded = user.depositerewarded.add(pending);
        user.rewardDebt = user.amount.mul(pool.acctokenPerShare).div(1e18);
    ...
    function safetokenTransfer(address token, address _to, uint256 _amount) internal {
        ...
        require(tokenBalance >= _amount, "no enough token");
        IERC20(token).transfer(_to, _amount);
    }

    function safeOKTTransfer(address payable _to, uint256 _amount) internal{
        uint256 OKTBalance = address(this).balance;
        require(OKTBalance >= _amount, "no enough token");
