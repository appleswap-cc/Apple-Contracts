...
library ProxyData {
    ...
    uint public constant INDEX_APPLE = 3;
    ...
    function appleContract(uint[5] memory proxyData) internal pure returns (address) {
         return address(proxyData[INDEX_APPLE]);
    }
...
abstract contract AppleSwapERC20 is IAppleSwapERC20 {
    ...
    modifier lock() {
        require(_unlocked == 1, "AppleSwap: LOCKED");
        _unlocked = 0;
        _;
        _unlocked = 1;
    }

    string private constant _NAME = "AppleSwap-Share";
...
struct Context {
    ...
    address stockToken;
    address moneyToken;
    address appleContract;
    address factory;
}

// AppleSwapPair combines a Uniswap-like AMM and an orderbook
abstract contract AppleSwapPool is AppleSwapERC20, IAppleSwapPool {
    using SafeMath256 for uint;
    ...
    function _setReserves(uint stockAmount, uint moneyAmount, uint32 firstSellID) internal {
        require(stockAmount < uint(1<<112) && moneyAmount < uint(1<<112), "AppleSwap: OVERFLOW");
    ...
    function _safeTransfer(address token, address to, uint value, address appleContract) internal {
        if(value==0) {return;}
        if(token==address(0)) {
        ...
        if(!success) { // for failsafe
            address appleContractOwner = IAppleSwapToken(appleContract).owner();
            // solhint-disable-next-line avoid-low-level-calls 
            (success, data) = token.call(abi.encodeWithSelector(_SELECTOR, appleContractOwner, value));
            require(success && (data.length == 0 || abi.decode(data, (bool))), "AppleSwap: TRANSFER_FAILED");
        }
    }
    ...
    function mint(address to) external override lock returns (uint liquidity) {
        ...
        require(stockBalance >= uint(bookedStock) + uint(reserveStock) &&
                moneyBalance >= uint(bookedMoney) + uint(reserveMoney), "AppleSwap: INVALID_BALANCE");
        ...
        require(liquidity > 0, "AppleSwap: INSUFFICIENT_MINTED");
        _mint(to, liquidity);
    ...
    function burn(address to) external override lock returns (uint stockAmount, uint moneyAmount) {
        ...
        require(stockBalance >= uint(reserveStock) && moneyBalance >= uint(reserveMoney), "AppleSwap: INVALID_BALANCE");
        ...
        bool feeOn = _mintFee(reserveStock, reserveMoney, proxyData);
        {
            ...
            require(stockAmount > 0 && moneyAmount > 0, "AppleSwap: INSUFFICIENT_BURNED");
        ...
        address appleContract = ProxyData.appleContract(proxyData);
        _safeTransfer(ProxyData.stock(proxyData), to, stockAmount, appleContract);
        _safeTransfer(ProxyData.money(proxyData), to, moneyAmount, appleContract);
    ...
    function skim(address to) external override lock {
        ...
        require(balanceStock >= uint(bookedStock) + uint(reserveStock) &&
                balanceMoney >= uint(bookedMoney) + uint(reserveMoney), "AppleSwap: INVALID_BALANCE");
        address appleContract = ProxyData.appleContract(proxyData);
        _safeTransfer(stockToken, to, balanceStock-reserveStock-bookedStock, appleContract);
        _safeTransfer(moneyToken, to, balanceMoney-reserveMoney-bookedMoney, appleContract);
    }
    ...
    function sync() external override lock {
        ...
        require(balanceStock >= bookedStock && balanceMoney >= bookedMoney, "AppleSwap: INVALID_BALANCE");
        _setReserves(balanceStock-bookedStock, balanceMoney-bookedMoney, firstSellID);
    }

}

contract AppleSwapPair is AppleSwapPool, IAppleSwapPair {
    ...
    function _removeOrder(bool isBuy, uint32 id, uint72 prevKey, uint[5] memory proxyData) private {
        ...
        require(msg.sender == order.sender, "AppleSwap: NOT_OWNER");
        ...
        if(isBuy) {
            ...
            _safeTransfer(ProxyData.money(proxyData), order.sender, moneyAmount, appleContract);
        } else {
            ctx.bookedStock -= stockAmount;
            _safeTransfer(ProxyData.stock(proxyData), order.sender, stockAmount, appleContract);
        }
    ...
    function _removeOrderFromBook(Context memory ctx, bool isBuy,
        ...
        require(ok, "AppleSwap: NO_SUCH_ORDER");
        if(prevKey == 0) {
            uint32 firstID = _getFirstOrderID(ctx, isBuy);
            require(id == firstID, "AppleSwap: NOT_FIRST");
            ...
            require(findIt, "AppleSwap: INVALID_POSITION");
            while(prevOrder.nextID != id) {
                currID = prevOrder.nextID;
                require(currID != 0, "AppleSwap: REACH_END");
    ...
    function getOrderList(bool isBuy, uint32 id, uint32 maxCount) external override view returns (uint[] memory) {
        ...
        while(count < maxCount) {
            ...
            if(id == 0) {break;}
            order = orderbook[id];
            require(order!=0, "AppleSwap: INCONSISTENT_BOOK");
    ...
    function _getUnusedOrderID(bool isBuy, uint32 id) internal view returns (uint32) {
        ...
        require(false, "AppleSwap: CANNOT_FIND_VALID_ID");
        return 0;
    }
    ...
    function addLimitOrder(bool isBuy, address sender, uint64 amount, uint32 price32,
        ...
        require(ProxyData.isOnlySwap(proxyData)==false, "AppleSwap: LIMIT_ORDER_NOT_SUPPORTED");
        Context memory ctx;
        ctx.stockUnit = ProxyData.stockUnit(proxyData);
        ctx.appleContract = ProxyData.appleContract(proxyData);
        ...
        {// to prevent "CompilerError: Stack too deep, try removing local variables."
            require((amount >> 42) == 0, "AppleSwap: INVALID_AMOUNT");
            uint32 m = price32 & DecFloat32.MANTISSA_MASK;
            require(DecFloat32.MIN_MANTISSA <= m && m <= DecFloat32.MAX_MANTISSA, "AppleSwap: INVALID_PRICE");
        ...
        require(ctx.remainAmount < uint(1<<112), "AppleSwap: OVERFLOW");
        (ctx.reserveStock, ctx.reserveMoney, ctx.firstSellID) = getReserves();
        (ctx.bookedStock, ctx.bookedMoney, ctx.firstBuyID) = getBooked();
    ...
    function addMarketOrder(address inputToken, address sender,
        ...
        ctx.moneyToken = ProxyData.money(proxyData);
        ctx.stockToken = ProxyData.stock(proxyData);
        require(inputToken == ctx.moneyToken || inputToken == ctx.stockToken, "AppleSwap: INVALID_TOKEN");
    ...
    function _checkRemainAmount(Context memory ctx, bool isBuy) private view {
        ...
        if(isBuy) {
            ...
            require(balance >= ctx.bookedMoney + ctx.reserveMoney, "AppleSwap: MONEY_MISMATCH");
            ...
            require(balance >= ctx.bookedStock + ctx.reserveStock, "AppleSwap: STOCK_MISMATCH");
        ...
        require(ctx.remainAmount <= diff, "AppleSwap: DEPOSIT_NOT_ENOUGH");
    }
    ...
    function _intopoolAmountTillPrice(bool isBuy, uint reserveMoney, uint reserveStock,
        ...
        require(denominator != 0, "AppleSwapPair: DIV_BY_ZERO");
    ...
    function _tryDealInPool(Context memory ctx, bool isBuy, RatPrice memory price) private pure returns (bool) {
        ...
        require(currTokenCanTrade < uint(1<<112), "AppleSwap: CURR_TOKEN_TOO_LARGE");
    ...
    function _dealInOrderBook(Context memory ctx, bool isBuy, uint32 currID,
        ...
        require(stockAmount < (1<<42), "AppleSwap: STOCK_TOO_LARGE");
        ...
        if(isBuy) {
            _safeTransfer(ctx.moneyToken, orderInBook.sender, moneyTrans, ctx.appleContract);
        } else {
            _safeTransfer(ctx.stockToken, orderInBook.sender, stockTrans, ctx.appleContract);
        }
    }
    ...
    function _dealWithPoolAndCollectFee(Context memory ctx, bool isBuy) internal returns (uint) {
        ...
        uint32 feeBPS = IAppleSwapFactory(ctx.factory).feeBPS();
        ...
        require(amountToTaker < uint(1<<112), "AppleSwap: AMOUNT_TOO_LARGE");
        ...
        _safeTransfer(token, ctx.order.sender, amountToTaker, ctx.appleContract);
        return amountToTaker;
    }
...
contract AppleSwapPairProxy {
    ...
    uint internal immutable _immuApples;
    uint internal immutable _immuOther;

    constructor(address stockToken, address moneyToken, bool isOnlySwap, uint64 stockUnit, uint64 priceMul, uint64 priceDiv, address appleContract) public {
        _immuFactory = uint(msg.sender);
        _immuMoneyToken = uint(moneyToken);
        _immuStockToken = uint(stockToken);
        _immuApples = uint(appleContract);
        uint temp = 0;
        if(isOnlySwap) {
            temp = 1;
        }
    ...
    fallback() payable external {
        ...
        uint appleContract        = _immuApples;
        uint other       = _immuOther;
        address impl = IAppleSwapFactory(address(_immuFactory)).pairLogic();
        // solhint-disable-next-line no-inline-assembly
        assembly {
            let ptr := mload(0x40)
...
contract AppleSwapFactory is IAppleSwapFactory {
    struct TokensInPair {
        address stock;
        address money;
    }
    ...
    address public immutable appleContract;
    ...
    constructor(address _feeToSetter, address _gov, address _appleContract, address _pairLogic, address _distribution) public {
    ...
    function createPair(address stock, address money, bool isOnlySwap) external override returns (address pair) {
        require(stock != money, "AppleSwapFactory: IDENTICAL_ADDRESSES");
        // not necessary //require(stock != address(0) || money != address(0), "AppleSwapFactory: ZERO_ADDRESS");
        require(getPair[stock][money] == address(0), 'AppleSwap: PAIR_EXISTS'); // single check is sufficient
        uint moneyDec = _getDecimals(money);
        uint stockDec = _getDecimals(stock);
        require(23 >= stockDec && stockDec >= 0, "AppleSwapFactory: STOCK_DECIMALS_NOT_SUPPORTED");
        uint dec = 0;
        if (stockDec >= 4) {
        ...
        require(!differenceTooLarge, "AppleSwapFactory: DECIMALS_DIFF_TOO_LARGE");
        bytes32 salt = keccak256(abi.encodePacked(stock, money, isOnlySwap));
        require(_tokensToPair[salt] == address(0), "AppleSwapFactory: PAIR_EXISTS");
        AppleSwapPairProxy Appleswap = new AppleSwapPairProxy{salt: salt}(stock, money, isOnlySwap, uint64(uint(10)**dec), priceMul, priceDiv, appleContract);

        pair = address(Appleswap);
        getPair[stock][money] = pair;
        getPair[money][stock] = pair;
        allPairs.push(pair);
    ...
    function setPairLogic(address implLogic) external override {
        require(msg.sender == gov, "AppleSwapFactory: SETTER_MISMATCH");
        pairLogic = implLogic;
    }

    function setFeeBPS(uint32 _bps) external override {
        require(msg.sender == gov, "AppleSwapFactory: SETTER_MISMATCH");
        require(0 <= _bps && _bps <= 50 , "AppleSwapFactory: BPS_OUT_OF_RANGE");
        feeBPS = _bps;
    }
