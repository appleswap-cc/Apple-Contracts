...
contract AppleSwapBuyback is IAppleSwapBuyback {
    ...
    address private constant _OKT = address(0);
    ...
    constructor(address _appleContract, address _router, address _factory) public {
        ...
        _mainTokens[_OKT] = true;
        _mainTokenArr.push(_OKT);
    ...
    function removeMainToken(address token) external override {
        require(msg.sender == IAppleSwapToken(appleContract).owner(), "AppleSwapBuyback: NOT_Apple_OWNER");
        require(token != _OKT, "AppleSwapBuyback: REMOVE_OKT_FROM_MAIN");
    ...
    function _swapForApples(address pair) private {
        ...
        if (token == _OKT) { // okt -> Apples
            uint256 oktAmt = address(this).balance;
            // require(oktAmt > 0, "AppleSwapBuyback: NO_OKT");
            if (oktAmt == 0) { return; }

            IAppleSwapRouter(router).swapToken{value: oktAmt}(
                _OKT, oktAmt, 0, path, address(this), _MAX_UINT256);
