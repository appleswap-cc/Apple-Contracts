...
interface IAppleExchange{
    function Exchange(address user, address path, address input, uint256 amountIn, uint256 amountOut) external returns(bool);
}
...
contract AppleSwapRouter is IAppleSwapRouter, Ownable {
    using SafeMath256 for uint;
    address public immutable override factory;
    address public swapmining;
    modifier ensure(uint deadline) {
        // solhint-disable-next-line not-rely-on-time,
        require(deadline >= block.timestamp, "AppleSwapRouter: EXPIRED");
        _;
    }
    ...
    function _addLiquidity(address pair, uint amountStockDesired, uint amountMoneyDesired,
        ...
        (uint reserveStock, uint reserveMoney, ) = IAppleSwapPool(pair).getReserves();
        if (reserveStock == 0 && reserveMoney == 0) {
            ...
            if (amountMoneyOptimal <= amountMoneyDesired) {
                require(amountMoneyOptimal >= amountMoneyMin, "AppleSwapRouter: INSUFFICIENT_MONEY_AMOUNT");
                ...
                require(amountStockOptimal >= amountStockMin, "AppleSwapRouter: INSUFFICIENT_STOCK_AMOUNT");
    ...
    function addLiquidity(address stock, address money, bool isOnlySwap, uint amountStockDesired,
        ...
        if (stock != address(0) && money != address(0)) {
            require(msg.value == 0, 'AppleSwapRouter: NOT_ENTER_ETH_VALUE');
        }
        address pair = IAppleSwapFactory(factory).tokensToPair(stock, money, isOnlySwap);
        if (pair == address(0)) {
            pair = IAppleSwapFactory(factory).createPair(stock, money, isOnlySwap);
        }
        ...
        liquidity = IAppleSwapPool(pair).mint(to);
    ...
    function _removeLiquidity(address pair, uint liquidity, uint amountStockMin,
        ...
        (amountStock, amountMoney) = IAppleSwapPool(pair).burn(to);
        require(amountStock >= amountStockMin, "AppleSwapRouter: INSUFFICIENT_STOCK_AMOUNT");
        require(amountMoney >= amountMoneyMin, "AppleSwapRouter: INSUFFICIENT_MONEY_AMOUNT");
    }
    ...
    function _swap(address input, uint amountIn, address[] memory path, address _to) internal virtual returns (uint[] memory amounts) {
        ...
        for (uint i = 0; i < path.length; i++) {
            ...
            amounts[i + 1] = IAppleSwapPair(path[i]).addMarketOrder(input, to, uint112(amounts[i]));
            if (swapmining != address(0)){
                IAppleExchange(swapmining).Exchange(msg.sender, path[i], input, amounts[i], amounts[i + 1]);
            }
            if (!isLastSwap) {
    ...
    function swapToken(address token, uint amountIn, uint amountOutMin, address[] calldata path,
        ...
        if (token != address(0)) { require(msg.value == 0, 'AppleSwapRouter: NOT_ENTER_ETH_VALUE'); }
        require(path.length >= 1, "AppleSwapRouter: INVALID_PATH");
        // ensure pair exist
        _getTokensFromPair(path[0]);
        _safeTransferFrom(token, msg.sender, path[0], amountIn);
        amounts = _swap(token, amountIn, path, to);
        require(amounts[path.length] >= amountOutMin, "AppleSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT");
    }
    ...
    function limitOrder(bool isBuy, address pair, uint prevKey, uint price, uint32 id,
        ...
        {
            (uint _stockAmount, uint _moneyAmount) = IAppleSwapPair(pair).calcStockAndMoney(uint64(stockAmount), uint32(price));
            if (isBuy) {
                if (money != address(0)) { require(msg.value == 0, 'AppleSwapRouter: NOT_ENTER_ETH_VALUE'); }
                _safeTransferFrom(money, msg.sender, pair, _moneyAmount);
            } else {
                if (stock != address(0)) { require(msg.value == 0, 'AppleSwapRouter: NOT_ENTER_ETH_VALUE'); }
                _safeTransferFrom(stock, msg.sender, pair, _stockAmount);
            }
        }
        IAppleSwapPair(pair).addLimitOrder(isBuy, msg.sender, uint64(stockAmount), uint32(price), id, uint72(prevKey));
    }
    ...
    function _safeTransferFrom(address token, address from, address to, uint value) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "AppleSwapRouter: TRANSFER_FROM_FAILED");
        uint afterAmount = IERC20(token).balanceOf(to);
        require(afterAmount == beforeAmount + value, "AppleSwapRouter: TRANSFER_FAILED");
    }
    ...
    function _quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, "AppleSwapRouter: INSUFFICIENT_AMOUNT");
        require(reserveA > 0 && reserveB > 0, "AppleSwapRouter: INSUFFICIENT_LIQUIDITY");
        amountB = amountA.mul(reserveB) / reserveA;
    }

    function _getTokensFromPair(address pair) internal view returns(address stock, address money) {
        (stock, money) = IAppleSwapFactory(factory).getTokensFromPair(pair);
        require(stock != address(0) || money != address(0), "AppleSwapRouter: PAIR_NOT_EXIST");
    }
    function query(address pair, uint256 amountIn, bool is_buy) public view returns(uint256) {
        (uint256 reserveStock, uint256 reserveMoney,) = IAppleSwapPool(pair).getReserves();
        if (is_buy){
            return amountIn.mul(reserveMoney).div(reserveStock.add(amountIn));
        }else{
            return amountIn.mul(reserveStock).div(reserveMoney.add(amountIn));
        }
    }
}
